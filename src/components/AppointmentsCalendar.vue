<template>
    <div id="printArea" :class="{printing:isPrinting}">
        <!-- top controls -->
        <div class="flex gap-2 w-max items-center mb-10"> 
            <h2 class="text-3xl font-bold mr-10">{{ dateFormat('%sm %y',new Date(cc.y,cc.m,cc.d).getTime()) }}</h2>
            <button @click="nextMonths(-1)" class="text-sm text-center flex justify-center items-center shadow shadow-gray-300 h-[30px] p-2 bg-gray-200 hover:scale-105 active:scale-95 transition"><i v-html="icons.chevronLeft" class="m-1 mt-[5px]"></i>Prev</button>
            <button class="text-sm text-center flex justify-center items-center shadow shadow-gray-300 h-[30px] p-2 bg-gray-200 hover:scale-105 active:scale-95 transition" @click="printCalendar">Print As PDF<i v-html="icons.pdf" class="m-1 mt-[5px]"></i></button>
            <button @click="nextMonths(1)" class="text-sm text-center flex justify-center items-center shadow shadow-gray-300 h-[30px] p-2 bg-gray-200 hover:scale-105 active:scale-95 transition">Next <i v-html="icons.chevronRight" class="m-1 mt-[5px]"></i></button>
            <div v-if="isPrinting">
                As of {{dateFormat('%sm %d, %y - %h:%I:%S %a',time) }}
            </div>
        </div>

        <!-- calendar view -->
        <div class="grid grid-cols-7 gap-2">
            <div class="pb-4">SUN</div>
            <div class="pb-4">MON</div>
            <div class="pb-4">TUE</div>
            <div class="pb-4">WED</div>
            <div class="pb-4">THU</div>
            <div class="pb-4">FRI</div>
            <div class="pb-4">SAT</div>

            <div class="pt-4 pb-8 border-t" v-for="c,ic in cBox" :class="{
                ...
                <div class="text-sm mb-2"  v-for="cs,i in c.scheds">
                    ...
                    <ul class="list-disc list-inside">
                        <li class="whitespace-nowrap overflow-hidden text-ellipsis hover:underline cursor-pointer" v-for="appt,ia in cs.appointments"
                            @click="modal.info = {...appt,...cs};modal.open=true"
                        >{{ appt.book_appointment_name==''? '<No Name Index>':appt.book_appointment_name }} ({{ ['Pending','Approved','Denied','Completed'][appt.book_appointment_status] }})</li>
                    </ul>
                </div>
            </div>
...
import { dateFormat, axios } from '../functions';
...
onMounted(()=>{
    ...
    setInterval(()=>time.value=new Date().getTime(),500)
})
...
function buildCalendar(){
    let startMonth = new Date(cc.y,cc.m,1);
    let endMonth = new Date(cc.y,cc.m+1,0);
    let offsetStart = new Date(cc.y,cc.m,1).getDay();
    let offsetEnd = new Date(cc.y,cc.m+1,0).getDay();
...
function printCalendar(){
    ...
    setTimeout(() => {
        let opt = {
            margin: [10,10,10,10],
            filename: `booking-schedule-${dateFormat('%sm-%y',new Date(cc.y,cc.m,cc.d).getTime())}-asof_${dateFormat('%y-%M-%D',new Date().getTime())}.pdf`,
...
function fetchSchedulesFromServer(){
    let start = dateFormat('%y-%M-%D', new Date(cc.y,cc.m-1,1).getTime())
    let end = dateFormat('%y-%M-%D', new Date(cc.y,cc.m+2,0).getTime())
    lastFetch.m = cc.m;
    lastFetch.y = cc.y;
...
<style scoped>
#printArea.printing * {font-family: sans-serif;font-size:12px !important;}
#printArea.printing h2{font-size:16px !important;margin-right: 15px;}
...
#printArea.printing ul li{white-space: normal;overflow: visible;text-overflow: unset;}
</style>